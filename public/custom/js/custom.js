...
(function(jQuery){
     ...
     */
    jQuery.gritter.remove = function(id, params){
     ...
     * @see Gritter#stop();
     */
    jQuery.gritter.removeAll = function(params){
        Gritter.stop(params || {});
    }
    ...
    var Gritter = {
        ...
        // Public - options to over-ride with jQuery.gritter.options in "add"
        position: '',
        fade_in_speed: '',
        fade_out_speed: '',
        time: '',
        ...
        add: function(params){
            ...
            var title = params.title,
                ...
                item_class = params.class_name || jQuery.gritter.options.class_name,
                position = jQuery.gritter.options.position,
            ...
            // Assign callbacks
            jQuery(['before_open', 'after_open', 'before_close', 'after_close']).each(function(i, val){
                Gritter['_' + val + '_' + number] = (jQuery.isFunction(params[val])) ? params[val] : function(){}
            });
            ...
            jQuery('#gritter-notice-wrapper').addClass(position).append(tmp);

            var item = jQuery('#gritter-item-' + this._item_count);

            item.fadeIn(this.fade_in_speed, function(){
                Gritter['_after_open_' + number](jQuery(this));
            });
            ...
            jQuery(item).bind('mouseenter mouseleave', function(event){
                if(event.type == 'mouseenter'){
                    if(!sticky){
                        Gritter._restoreItemIfFading(jQuery(this), number);
                    }
                }
                else {
                    if(!sticky){
                        Gritter._setFadeTimer(jQuery(this), number);
                    }
                }
                Gritter._hoverState(jQuery(this), event.type);
            });

            // Clicking (X) makes the perdy thing close
            jQuery(item).find('.gritter-close').click(function(){
        ...
        _countRemoveWrapper: function(unique_id, e, manual_close){
            ...
            if(jQuery('.gritter-item-wrapper').length == 0){
        ...
        removeSpecific: function(unique_id, params, e, unbind_events){
            ...
            if(!e){
                var e = jQuery('#gritter-item-' + unique_id);
            }
        ...
        _runSetup: function(){
            ...
            for(opt in jQuery.gritter.options){
                this[opt] = jQuery.gritter.options[opt];
            }
            this._is_setup = 1;
        ...
        stop: function(params){
            ...
            var wrap = jQuery('#gritter-notice-wrapper');
            before_close(wrap);
        ...
        _verifyWrapper: function(){
            ...
            if(jQuery('#gritter-notice-wrapper').length == 0){
                jQuery('body').append(this._tpl_wrap);
            }
...
function getCommentList(){
    jQuery.ajax({
        ...
        type: 'GET',
        data: {
            fertilizer_id: jQuery('#fertilizer_id').val(),
            last_comment_id: jQuery('.comment_id:last').val()
        },
...
function addComment2() {
    ...
    jQuery.ajax({
        ...
        data: {
            comment_text: jQuery('#message').val(),
            fertilizer_id: jQuery('#fertilizer_id').val(),
...
function showRequest1() {
    jQuery('.blur').fadeIn(100);
    jQuery('#requestForm').fadeIn(100);
}

function closeRequest() {
    jQuery('#requestForm').fadeOut(100);
    jQuery('.blur').fadeOut(100);
}
...
function addRequest(){
    ...
    jQuery.ajax({
        ...
        data: {
            ...
            user_name: jQuery('#request_user_name').val(),
